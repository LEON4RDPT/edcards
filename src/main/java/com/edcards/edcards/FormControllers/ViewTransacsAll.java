...
public class ViewTransacsAll  {
    ...
    @FXML
    private TableColumn<Transacao, Integer> idColumn;
    @FXML
    private TableColumn<Transacao, String> clientNameColumn;
    @FXML
    private TableColumn<Transacao, String> workerNameColumn;
    @FXML
    ...
    private void initialize() {
        ...
        var trans = TransacaoBLL.getAllTransacoes();
        if (trans == null) {
            //feedback;
        } else {
            transactions.addAll(trans);
            idColumn.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getIdTransacao()));
            workerNameColumn.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getFuncionario().getNome()));
            clientNameColumn.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getCliente().getNome()));
